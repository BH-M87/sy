...
use common\services\small\VoteService;
use Yii;
use common\libs\F;
...
class VoteController extends BaseController {
    ...
    public function actionView()
    {
        ...
        $appUserId   = F::value($this->params, 'user_id', 0);
        $room_id = F::value($this->params, 'room_id', 0);
        if (!$voteId) {
        ...
        if (!$room_id) {
            return F::apiFailed("房号id不能为空！");
        }
        $result = VoteService::service()->voteView($voteId, $appUserId, $room_id);
        if($result['errCode']) {
    ...
    public function actionDoVote()
    {
        ...
        $result = VoteService::service()->doVote($voteId, $voteDet, $appUserId, $communityId, $room_id);
