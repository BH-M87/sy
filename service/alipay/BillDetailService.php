...
//        }
        if (!empty($requestArr['pay_type'])){//20170716 报事报修支持支付方式搜索
            $where .= " AND der.pay_type=:pay_type";
            $params = array_merge($params, [':pay_type' => $requestArr["pay_type"]]);
        }
...
//            $params = array_merge($params, [':unit' => $requestArr["unit_id"]]);
//        }
        if (!empty($requestArr["building_id"])) {
...
//                $sql = "select  distinct bill.id,bill.acct_period_start,bill.acct_period_end,bill.cost_name,der.trade_no,der.community_id,bill.room_status,bill.`group`,bill.building,bill.unit,bill.room,der.pay_amount as total_amount,der.pay_channel,der.buyer_account,der.pay_time from ps_bill as bill, ps_order  as der where bill.order_id=der.id and der.bill_id=bill.id and der.status=2 and der.pay_status=1 and der.is_del=1  " . $where . " group by bill.id order by  der.pay_time desc limit $limit,$rows ";
                $sql = "select  distinct bill.id,bill.community_name,bill.room_address,bill.acct_period_start,bill.acct_period_end,bill.cost_name,der.trade_no,der.community_id,bill.room_status,bill.`group_id`,bill.building_id,bill.unit_id,bill.room_id,der.pay_amount as total_amount,der.pay_channel,der.buyer_account,der.pay_time,bill.trade_type,bill.trade_remark,der.remark from ps_bill as bill, ps_order  as der where bill.order_id=der.id and der.bill_id=bill.id and (der.status=2 or der.status=7) and der.pay_status=1 and der.is_del=1  " . $where . " group by bill.id order by  der.pay_time desc limit $limit,$rows ";
                break;
        }
