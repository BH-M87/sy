...
//            return $this->success(['totals' => 0, 'total_money' => 0, 'list' => [], "reportData" => $reportData]);
        ...
        switch ($requestArr['source']) {
            case 1://线上缴费
                $sql = "select  distinct bill.id,bill.room_address,bill.acct_period_start,bill.acct_period_end,bill.cost_name,der.trade_no,der.community_id,bill.room_status,bill.`group_id`,bill.building_id,bill.unit_id,bill.room_id,der.pay_amount as total_amount,der.pay_channel,der.buyer_account,der.pay_time,bill.trade_type,bill.trade_remark from ps_bill as bill, ps_order  as der where bill.order_id=der.id and der.bill_id=bill.id and der.status=2 and der.pay_status=1 and der.is_del=1  " . $where . " group by bill.id order by  der.pay_time desc limit $limit,$rows ";
                break;
            case 2://扫码支付
