...
use service\alipay\AppWebService;
...
class ParkFeeService extends BaseService
{
    public function getFee($reqArr)
    {
        ...
        //存入一条查询记录，供查询用
        $lkPayCodeModel = new ParkingLkPayCode();
    ...
    {
        //费用记录
        $payCodeLk = ParkingLkPayCode::find()
        ...
        if ($reqArr['pay_from'] == 2) {
            ...
            $codelkModel = ParkingLkPayCode::findOne($reqArr['out_id']);
    ...
    public function markPay($reqArr)
    {
        ...
        $data['buyer_id'] = '';
        //生成订单
        AppWebService::generalBill($data);
        $payCodeLk = ParkingLkPayCode::find()
            ->where(['id' => $reqArr['out_id']])
            ->asArray()
            ->one();
