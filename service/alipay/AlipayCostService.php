...
//        $roomUser = PsRoomUser::find()->where(['room_id' => $roomData['room_id'], 'identity_type' => 1])->select('name')->asArray()->column();
        ...
        //查询语句sql
        $sql = "select  bill.id as bill_id,bill.cost_id,bill.cost_type,bill.cost_name,bill.bill_entry_amount,bill.paid_entry_amount,bill.prefer_entry_amount,bill.acct_period_start,bill.acct_period_end,der.pay_channel from ps_bill as bill,ps_order  as der where {$where}   order by bill.acct_period_start desc;";
        $models = Yii::$app->db->createCommand($sql, $params)->queryAll();
        foreach ($models as $key => $model) {
            $arr = [];
            ...
            $arr['cost_type'] = $model['cost_type'];    //收费类型
            $arr['cost_name'] = $model['cost_name'];    //收费项名称
            $arr['bill_all_amount'] = $model['bill_entry_amount'];    //账单总金额
            //应收金额=账单金总金额减去预收金额
            $payResult =self::getBillPayAmount($model['bill_entry_amount'],$calc_recharge_amount);
            $arr['bill_entry_amount'] = $payResult['pay_amount'];//账单应缴金额
            $arr['bill_recharge_amount'] = $model['bill_entry_amount']-$payResult['pay_amount'];//剩余预存抵扣金额
            //剩余可使用的预存
