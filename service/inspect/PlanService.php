...
class PlanService extends BaseService
{
    ...
    protected static function checkCommon($params, $userInfo = [], $scenario)
    {
        ...
        //查看巡检线路是否存在
        $line = LineService::lineOne($params['line_id'], 'id');
        if (empty($line)) {
    ...
    public static function planOne($id = '', $name = '', $community_id = '', $select)
    {
        $select = $select ?? "id";
        return PsInspectPlan::find()
            ->select($select)
            ->andFilterWhere(['name' => $name, 'community_id' => $community_id, 'id' => $id])
            ->one();
    }
    //获取执行时间-编辑页面专用
