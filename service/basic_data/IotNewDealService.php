...
class IotNewDealService extends BaseService
{
    ...
    public function dealUserToIot($data,$type,$status='')
    {
        ...
        if(!empty($paramData)){
            //todo 写入redis
            Yii::$app->redis->rpush(YII_PROJECT.YII_ENV."IotMqData_sqwn",json_encode($paramData));
        }

        if(!empty($postData)){
            //todo 写入redis
            Yii::$app->redis->rpush(YII_PROJECT.YII_ENV."IotMqData_sqwn",json_encode($postData));
        }
