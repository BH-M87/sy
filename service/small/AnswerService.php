...
Class AnswerService extends BaseService
{
    ...
    public $start = '2019-11-05 08:10';
    public $end = '2019-11-14 18:00';
    ...
    public $choose_answer_list = [
        ...
        [
            ...
            'answer' => ['用鼻子闻','用火试','用肥皂水刷到软管和接口处检查'],
            'right' => '2',
            'type' => '1'
        ],
        [
            'problem' => '当打开房门闻到很浓的燃气气味时，要迅速( )，防止引发爆燃事故',
            'answer' => ['打开电灯查找漏气部位','打开门窗通风','点火查看'],
            'right' => '1',
            'type' => '1'
        ],
        [
            'problem' => '家庭中使用燃气设施和用具时，正确的做法是( )',
            'answer' => ['自行更换、拆改燃气的管道','燃气使用完毕后不用关闭总阀门','经常检查燃气灶具及管道，不擅自安装、拆改','使用液化气罐时罐内残留的液体可以倒在下水道里'],
            'right' => '2',
            'type' => '1'
        ],
        [
            'problem' => '居民家庭应配备哪种消防器材( )',
            'answer' => ['沙土、水缸、水桶','逃生绳','防毒面具'],
            'right' => '0',
            'type' => '1'
        ],
        [
            'problem' => '发生火灾逃生时，要尽量贴近地面撤离，主要原因是( )',
            'answer' => ['看得清地上有无障碍物','燃烧产生的有毒热烟在离地面近的地方浓度较小，可降低中毒几率','以免碰着别人'],
            'right' => '1',
            'type' => '1'
        ],
        [
            'problem' => '由于行为人的过失引起火灾，造成严重后果的行为，构成( )',
            'answer' => ['纵火罪','失火罪','玩忽职守罪'],
            'right' => '1',
            'type' => '1'
        ],
        [
            'problem' => '我国每年的“119”消防宣传活动日是( )',
            'answer' => [' 11月9日','1月19日','9月11日'],
            'right' => '0',
            'type' => '1'
        ],
        [
            'problem' => '遇到火灾时，正确的做法是( )',
            'answer' => ['沉着、冷静，迅速正确逃生','不用着急，等待消防队救援','迅速坐电梯逃生'],
            'right' => '0',
            'type' => '1'
        ],
        [
            'problem' => '如果睡觉时被烟火呛醒，正确的逃生方式是( )',
            'answer' => ['立即寻找逃生通道，迅速逃生','往床底下钻','穿好衣服再走'],
            'right' => '0',
            'type' => '1'
        ],
        [
            'problem' => '发生火灾后，如何正确报火警( )',
            'answer' => ['讲清着火单位、详细地址、着火物质及火势大小','讲清着火单位、详细地址、着火物质及火势大小，是否有人被困，留下报警人 姓名及联系方式 ','告知着火方位后迅速挂断电话，返回火场'],
            'right' => '1',
            'type' => '1'
        ],
        [
            'problem' => '单位发生火灾，你首先应( )',
            'answer' => ['及时拨打119火警电话并通知单位消防负责人','先自行扑救，救不了时再报火警','只拨打单位内部报警电话，不拨打119火警电话'],
            'right' => '0',
            'type' => '1'
        ],
        [
        ...
        [
            ...
            'answer' => ['收取成本费用','按照出动车辆数和扑救时间收费','对单位收费，对个人不收费','不收取任何费用'],
            'right' => '3',
            'type' => '1'
        ],
        [
            'problem' => '个人损坏、挪用或擅自拆除、停用消防设施、器材，埋压、圈占、遮挡消火栓的， 处( )处罚',
            'answer' => ['警告或五百元以下罚款','十日以下行政拘留','劳动教养'],
            'right' => '0',
            'type' => '1'
        ],
    ];
    ...
    public function addGrade($params)
    {
        ...
        if (!$answer->save()) {
            throw new MyException($answer->getErrors());
        }
    }
    ...
    public function getTopInfo($params)
    {
        ...
        try {
            ...
            if (!empty($user['app_user_id'])) {
                ...
                $own['avatar'] = $app_user['avatar'] == 'no-header-img' ? '' : $app_user['avatar'];
            }
        ...
        foreach ($list as &$v) {
            ...
            $v['avatar'] = $app_user['avatar'] == 'no-header-img' ? '' : $app_user['avatar'];;
        }
    ...
    {
        $all = Answer::find()->orderBy('grade desc,created_at asc');
        if ($limit) {
