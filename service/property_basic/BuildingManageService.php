...
use services\BuildingCenterService;
...
use app\modules\property\services\DoorPushService;

class BuildingManageService extends BaseService
{
    ...
    public function add($data, $postfix = true)
    {
        ...
        if (empty($group_id)) {
            $res = GroupManageService::service()->saveGroupDefault($community_id);
            if ($res['code']) {
                $group_id = $res['data'];
            } else {
    ...
    public function edit($data,$userinfo='')
    {
        ...
        if (empty($unit)) {
            return PsCommon::responseAppFailed("单元ID不存在");
        }
        $model = $unit;

        $group = $this->checkBuilding(3, $community_id, $group_id, $building_code, '',$unit->building_id);
        if (!empty($group)) {
            return PsCommon::responseAppFailed("楼幢编号已存在");
        }
        ...
        if (!empty($group)) {
            return PsCommon::responseAppFailed("单元编号已存在");
        }
    ...
    public function detail($data)
    {
        ...
        if (empty($group)) {
            return PsCommon::responseAppFailed("单元ID不存在存在");
        }
    ...
    public function delete($data,$userinfo='')
    {
        ...
        if ($roomInfo) {
            return PsCommon::responseAppFailed("无挂靠房屋才可删除");
        }
    ...
    public function batch_add($data,$userinfo='')
    {
        ...
        foreach ($data['unit'] as $key => $value) {
            ...
            if (empty($building_name)) {
                return PsCommon::responseAppFailed("楼宇不能为空");
            }
            preg_match(Regular::string(1,20),$building_name,$test_build);
            if(!$test_build){
                return PsCommon::responseAppFailed("楼宇格式有误");
            }
            if (empty($unit_name)) {
                return PsCommon::responseAppFailed("单元不能为空");
            }
            preg_match(Regular::string(1,20),$unit_name,$test_unit);
            if(!$test_unit){
                return PsCommon::responseAppFailed("单元格式有误");
            }
        ...
        //判断是否存在重复的数组
        if ($repeat) {
            return PsCommon::responseAppFailed($repeat,50003);
        } else {
            $unitData = [];
            foreach ($newList as $k => $v) {
                ...
                if ($building) {
                    ...
                    if ($model->save()) {
                        ...
                        return PsCommon::responseAppFailed("楼宇新增失败");
                    }
                }
                foreach ($v as $z) {
            ...
            return PsCommon::responseAppSuccess("新增成功");
        }
    }
}