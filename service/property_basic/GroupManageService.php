 ...
 * Time: 9:47
 */
namespace app\modules\vistor\services;

use app\common\core\PsCommon;

use app\modules\property\services\OperateService;
use app\services\BuildingCenterService;

use app\modules\property\models\PsCommunityBuilding;
use app\modules\property\models\PsCommunityGroups;
use app\modules\property\models\PsCommunityModel;

class GroupManageService extends BaseService
{
    ...
    public function add($data,$userinfo)
    {
        ...
        if (!empty($group)) {
            return PsCommon::responseAppFailed("苑期区名称已存在");
        }

        $data['group_code'] = $data['group_code'] ? $this->fill_zero($data['group_code']) : 0;
        $group = $this->checkGroups($data['group_code'], 3, $data['community_id']);
        if (!empty($group)) {
            return PsCommon::responseAppFailed("苑期区编码已存在");
        }
        ...
        if ($res['code']) {
            ...
            return PsCommon::responseAppSuccess($res['data']);
        } else {
            return PsCommon::responseAppFailed($res['msg']);
        }
    }
    ...
    public function saveGroup($data)
    {
        ...
        if ($model->save()) {
            ...
            $send = [
                ...
                'communityCode' => PsCommunityModel::find()->select('community_code')->where(['id' => $model->community_id])->asArray()->scalar(),
            ];

            $result = BuildingCenterService::service(2)->request('/housecenter/group/addGroup', $send);
            if ($result["code"] == 1) {
                $model->groups_code = $result['data']['code'];
                $model->save();
            }
    ...
    public function edit($data,$userinfo)
    {
        ...
        if (empty($group)) {
            return PsCommon::responseAppFailed("苑期区ID不存在");
        }
        ...
        if ($model->save()) {
            ...
            return PsCommon::responseAppSuccess($model->id);
        } else {
            return PsCommon::responseAppFailed("编辑失败");
        }
    ...
    public function detail($data)
    {
        ...
        return PsCommon::responseAppSuccess($detail);
    }
    ...
    public function delete($data,$userinfo)
    {
        ...
        if ($build) {
            return PsCommon::responseAppFailed("区域下无挂靠楼宇才可删除");
        }
        
        // 删除 楼宇中心数据
        if (!empty($group->groups_code)) {
            $send = ['groupCode' => $group->groups_code];
            $result = BuildingCenterService::service(2)->request('/housecenter/group/deleteGroup', $send);
        } else {
            $result["code"] = 1;
        }
        ...
        if ($result["code"] == 1) {
            if ($group->delete()) {
                ...
                return PsCommon::responseAppSuccess("删除成功");
            } else {
                return PsCommon::responseAppFailed("删除失败");
            }
        } else {
            return PsCommon::responseAppFailed("删除失败！");
        }
