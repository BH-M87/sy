CREATE TABLE `ps_event` (
  ...
  `event_img` varchar(500) DEFAULT NULL COMMENT '事件照片',
  `status` tinyint(2) DEFAULT '1' COMMENT '状态：1待处理，2处理中，3已办结，4已驳回',
  `is_colse` tinyint(2) DEFAULT '1' COMMENT '是否结案：1未结案，2已结案',
...
CREATE TABLE `ps_event_process` (
  ...
  `content` varchar(500) DEFAULT NULL COMMENT '处置内容',
  `process_img` varchar(500) DEFAULT NULL COMMENT '处置图片',
